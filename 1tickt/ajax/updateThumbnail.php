...
if(isset($_POST['videoId']) && isset($_POST['thumbnailId'])) {
    ...
    $query = $connectionString->prepare("UPDATE thumbnails SET selected=0 WHERE videoId=:videoId");
    $query->bindParam(":videoId", $videoId);
    $query->execute();

    $query = $connectionString->prepare("UPDATE thumbnails SET selected=1 WHERE id=:thumbnailId");
